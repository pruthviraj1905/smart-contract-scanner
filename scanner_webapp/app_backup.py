...
class ScanManager:
    ...
    def _add_output(self, scan_id, message):
        """Add terminal output message to scan"""
        from datetime import datetime
        timestamp = datetime.now().strftime('%H:%M:%S')
    ...
    def _validate_contract_on_chain(self, contract_address, chain_config, api_key):
        """Validate if contract exists on the selected chain and get balance info"""
        import requests
        
        try:
            # Check if contract has code
            url = chain_config['api_base']
            params = {
                'module': 'proxy',
                'action': 'eth_getCode',
            ...
            contract_code = data.get('result', '0x')
            exists = contract_code != '0x' and len(contract_code) > 2
            ...
            if balance_data.get('result'):
                ...
                if eth_balance == 0:
                    validation_result['native_balance'] = '0'
                elif eth_balance < 0.001:
                    validation_result['native_balance'] = f'{wei_balance} wei'
                else:
                    validation_result['native_balance'] = f'{eth_balance:.6f} {self._get_native_symbol(chain_config["name"])}'
            
            # Get token balances (simplified - just check for common ERC20 transfers)
            try:
                # Get recent transactions to identify tokens
                tx_params = {
                    'module': 'account',
                    'action': 'tokentx',
                    'address': contract_address,
                ...
                if tx_data.get('result') and isinstance(tx_data['result'], list):
                    seen_tokens = set()
                    for tx in tx_data['result'][:5]:  # Check first 5 token transactions
                        token_symbol = tx.get('tokenSymbol', 'UNKNOWN')
                        token_address = tx.get('contractAddress')
                        
                        if token_address and token_address not in seen_tokens:
                            seen_tokens.add(token_address)
                            # Try to get current balance for this token
                            token_balance = self._get_token_balance(contract_address, token_address, chain_config, api_key)
                            if token_balance:
                                validation_result['token_balances'].append({
                                    'symbol': token_symbol,
                ...
                pass  # Token balance checking is optional
